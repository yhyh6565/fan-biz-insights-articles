...
print('🔍 Article 15 분석 중...\n')

# Get content
response = supabase.table('articles').select('content').eq('article_number', 15).single().execute()
content = response.data['content']

# Save to file for inspection
with open('/tmp/article15_content.txt', 'w', encoding='utf-8') as f:
    f.write(content)
...
print(f'1️⃣ Leading space 문제 (4칸 이상): {len(leading_space_issues)}개')
...
print(f'2️⃣ 이미지: 총 {len(image_lines)}개')
...
print(f'\n💾 전체 내용이 /tmp/article15_content.txt에 저장되었습니다.')
