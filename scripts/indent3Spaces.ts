...
async function indent3Spaces() {
  console.log('📝 모든 글의 불렛 포인트에 3칸 인덴트 추가 중...\n');
  ...
  for (const article of articles) {
    ...
    if (modified) {
      ...
      if (updateError) {
        ...
        console.log(`✅ [${article.article_number}] "${article.title}" - ${bulletCount}개 불렛에 3칸 인덴트 추가`);
      }
    } else {
      console.log(`⏭️  [${article.article_number}] "${article.title}" - 불렛 없음`);
    }
  }
