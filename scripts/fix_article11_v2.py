...
# Fix by finding line with 4 leading spaces and ![
lines = content.split('\n')

for i, line in enumerate(lines):
    leading_spaces = len(line) - len(line.lstrip())

    # Find line with exactly 4 leading spaces and image syntax
    if leading_spaces == 4 and line.strip().startswith('![') and '.jpg' in line:
        # Replace with corrected image
        lines[i] = f'![NCT 응원봉]({public_url})'
        print(f'✅ 라인 {i+1}: 이미지 수정 및 leading spaces 제거')
...
for i, line in enumerate(lines, 1):
    ...
    if leading_spaces >= 4 and line.strip():
        print(f'⚠️  라인 {i}에 여전히 {leading_spaces}개의 leading spaces: {line[:60]}...')
        has_issues = True
